...
| 1 | **Airbnb iCal two-way sync** — Airbnb exports each listing calendar as an iCal URL and imports external `.ics` feeds, auto-blocking their dates. The agent publishes a feed per unit (`automation/calendar/<unit>.ics`, statically hosted); Airbnb polls it every few hours. **FF already imports the Airbnb calendar link** (confirmed live), so blocks propagate transitively: our feed → Airbnb → FF | Free, ToS-clean | Automated block-before-yes on Airbnb + booking reads, inherited by FF. No clicks, no browser |
