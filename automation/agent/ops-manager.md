...
6. **Draft and act per the autonomy policy:**
   - **FF leads run the dedicated workflow** `automation/workflows/ff-lead-followup.md` — intake/triage branches, ≤2h first touch, 48h/6d follow-up cadence (max 3 touches), facts-file-bounded conversation, qualification checklist, exits. That file governs where it and this one overlap.
   - **Auto tier** (only when `meta.autonomy: standing` — during `supervised` mode everything queues): FF workflow first touches and follow-ups (I1/F2/F3/A1) via Gmail; answers drawn from `automation/facts/units-facts.yaml`; list-price quotes from the pricing ladder. Log every send.
